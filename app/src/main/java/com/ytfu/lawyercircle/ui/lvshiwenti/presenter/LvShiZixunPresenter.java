...
/** @Auther gxy @Date 2020/5/25 @Des 律师端 问题列表p */
public class LvShiZixunPresenter
        extends BaseRefreshPresenter<BaseRefreshView<LawyerConsultingBean.ListBean>> {

    public void getLvShiZiXunList(boolean isLoadMore, int page, String uid) {
        HttpUtil.getInstance()
                .getApiService()
                .setLawyerZiXunList(page, uid)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .as(bindLifecycle())
                .subscribe(
                        new BaseRxObserver<LawyerConsultingBean>() {
                            @Override
                            public void onNextImpl(LawyerConsultingBean lvShiZiXunListBean) {
                                ...
                                if (status == 1) {
                                    List<LawyerConsultingBean.ListBean> list =
                                            lvShiZiXunListBean.getList();
                                    if (isLoadMore) {
    ...
    public void getQuickAnswer(int position, String uid, String lsid, String consult_id) {
        Observable<QuickAnswerBean> observable =
                createService(ApiService.class).setQuickAnswer(uid, lsid, consult_id);
        requestRemote(
                observable,
                new BaseRxObserver<QuickAnswerBean>() {
                    @Override
